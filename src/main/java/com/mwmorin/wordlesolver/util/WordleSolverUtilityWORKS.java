...
public class WordleSolverUtilityWORKS {

	private static final Logger LOGGER = Logger.getLogger(WordleSolverUtilityWORKS.class.getName());
...
/*		try (InputStream input = new FileInputStream("/config.properties")) { // NOT FINDING FILE !
	...
	public WordleSolverUtilityWORKS(String sessionId) {
		super();
...
//		private static boolean allLettersAreGreenOrYellow = false;
	...
	private static void loadfiveLetterWordsAndWeights() {
		...
		File file = null;
		try {
			file = new ClassPathResource(fiveLetterWordsAndWeightsFilePathName).getFile();
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
		if (!file.exists()) {
